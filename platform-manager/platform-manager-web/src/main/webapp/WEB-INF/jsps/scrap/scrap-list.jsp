...
<script type="text/javascript">
	...
	/*带条件的分页查询 */
 	function jump(currentpage,rows,searchparam){
		...
		// 定义访问路径
		var url = "/scrap/scraplist?currentPage="+currentpage;
	...
	function delScrap(){
		...
		$.ajax({
			type : 'POST',
			url : "${pageContext.request.contextPath }/scrap/delete",
			data : { "ids[]" : ids },
...
<form action="/scrap/scraplist" method="post">
		...
		<div class="container">
			...
			<span class="input-group col-md-3  navbar-left"
				...
				class="input-group-btn"> <a href="#" class="btn btn-info"
					role="button"  onclick="delScarp();">删除</a> <a href="/main/scrap/scrap-add"
	...
	<table class="table  table-bordered  table-hover">
		...
		<tbody>
			<c:forEach items="${pageBean.rows}" var="scrap">
				<tr> 
					<td align="center"><input type="checkbox" name="ids"
						value="${scrap.id}"></td> 
						<td>${scrap.scrapCode}</td>
						<td>${scrap.scrapName}</td>
						<td>${scrap.category}</td>
						<td>${scrap.price}/${scrap.unit}</td>
						<td>${scrap.score}</td>
					<td><a href="/scrap/predit?id=${scrap.id}">修改</a>  </td>
				</tr>
			</c:forEach>
