...
<script type="text/javascript">
	...
	/*带条件的分页查询 */
 	function jump(currentpage,rows,searchparam){
		...
		// 定义访问路径
		var url = "/dict/dictlist?currentPage="+currentpage;
	...
	function delDict(){
		...
		$.ajax({
			type : 'POST',
			url : "${pageContext.request.contextPath }/dict/delete",
			data : { "ids[]" : ids },
...
<form action="/dict/dictlist" method="post">
		...
		<div class="container">
			...
			<span class="input-group col-md-3  navbar-left"
				...
				class="input-group-btn"> <a href="#" class="btn btn-info"
					role="button"  onclick="delDict();">删除</a> <a href="/main/dict/dict-add"
	...
	<table class="table  table-bordered  table-hover">
		...
		<tbody>
			<c:forEach items="${pageBean.rows }" var="sysdict">
				<tr>
		
					<td align="center"><input type="checkbox" name="ids"
						value="${sysdict.id}"></td> 
						<td>${sysdict.dictOption}</td>
						<td>${sysdict.dictText}</td>
						<td>${sysdict.dictSort}</td>
						<td>${sysdict.dictTabname}.${sysdict.dictColname}</td>
						<td>${sysdict.dictName}</td>
					<td><a href="/dict/predit?id=${sysdict.id}">修改</a>  </td>
				</tr>
			</c:forEach>
